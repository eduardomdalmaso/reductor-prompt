...
Converte as fontes SGML oficiais em Markdown e PDF estruturado para o ReductorPrompt Database.
"""
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        doc_title = getattr(self, 'doc_title', 'PostgreSQL Official Documentation')
        ...
        self.drawString(54, 32, "ReductorPrompt Database — PostgreSQL Official Source")
...
def sgml_node_to_markdown(node, depth=1) -> str:
    """Converte nós SGML/DocBook em Markdown limpo"""
    ...
    if tag_name in ['chapter', 'sect1', 'sect2', 'sect3', 'sect4', 'refsect1', 'refsect2']:
        res = []
        for child in node.children:
            res.append(sgml_node_to_markdown(child, depth=depth + 1))
        ...
        inner = []
        for child in node.children:
            inner.append(sgml_inline_to_markdown(child))
        return f"\n{''.join(inner).strip()}\n"
        ...
        for idx, li in enumerate(node.find_all('listitem', recursive=False), 1):
            ...
            for c in li.children:
                li_inner.append(sgml_node_to_markdown(c, depth=depth))
        ...
        res = []
        for child in node.children:
            res.append(sgml_node_to_markdown(child, depth=depth))
        return "".join(res)


def sgml_inline_to_markdown(node) -> str:
    if isinstance(node, NavigableString):
        return str(node)
    if not isinstance(node, Tag):
        return ""
...
def compile_postgres_official():
    print("Compilando Documentação Oficial do PostgreSQL (SGML Source)...")

    # Tópicos fundamentais de arquitetura, SQL, índices, concorrência e replicação
    target_sgmls = [
        ("SQL Syntax & Structure", "syntax.sgml"),
        ("Advanced Queries & CTEs", "queries.sgml"),
        ("Advanced Features", "advanced.sgml"),
        ("Data Types", "datatype.sgml"),
        ("JSON & JSONB", "json.sgml"),
        ("Arrays", "array.sgml"),
        ("Indexes & Execution Strategies", "indices.sgml"),
        ("B-Tree Index Internals", "btree.sgml"),
        ("GIN (Generalized Inverted Index)", "gin.sgml"),
        ("GiST (Generalized Search Tree)", "gist.sgml"),
        ("BRIN (Block Range Index)", "brin.sgml"),
        ("Concurrency Control & MVCC", "mvcc.sgml"),
        ("Performance Tips & Tuning", "perform.sgml"),
        ("Parallel Query Execution", "parallel.sgml"),
        ("Write-Ahead Logging (WAL)", "wal.sgml"),
        ("High Availability & Replication", "high-availability.sgml"),
        ("Server Configuration", "config.sgml"),
        ("Monitoring & Statistics", "monitoring.sgml"),
        ("Row Security Policies (RLS)", "ddl.sgml"),
    ]

    sections = []
    full_md = [
        "# PostgreSQL Official Documentation: Architecture, SQL Engine & Administration",
        "\n> **Autor:** PostgreSQL Global Development Group\n> **Fonte:** github.com/postgres/postgres/tree/master/doc\n\n---\n"
    ]

    for title, fname in target_sgmls:
        fpath = os.path.join(SGML_DIR, fname)
        ...
        md_text = sgml_node_to_markdown(soup)
        sections.append({'title': title, 'markdown': md_text})
        full_md.append(f"# {title}\n\n{md_text}\n\n---\n")

    out_md = os.path.join(DB_DIR, "postgresql_official_documentation.md")
    out_pdf = os.path.join(DB_DIR, "postgresql_official_documentation.pdf")
    ...
    render_book_pdf(
        ...
        "Architecture, SQL Syntax, Index Internals (B-Tree, GIN, GiST, BRIN), MVCC, WAL & Replication",
        "PostgreSQL Global Development Group",
        "https://github.com/postgres/postgres/tree/master/doc",
        sections,
        out_pdf
    )
