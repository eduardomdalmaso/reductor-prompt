...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        doc_title = getattr(self, 'doc_title', 'MinIO High-Performance Storage Guide')
        ...
        self.drawString(54, 32, "ReductorPrompt Database — Object Storage Collection")
...
def render_book_pdf(book_title, subtitle, author, meta_origin, sections, output_pdf):
    ...
    story.append(Paragraph("<b>Projeto:</b> ReductorPrompt Object Storage Collection", styles['CoverMeta']))
...
def compile_minio():
    print("Compilando MinIO High-Performance Object Storage Documentation...")
    sections = []
    full_md = [
        "# MinIO: High-Performance S3-Compatible Object Storage Architecture & Operations",
        "\n> **Autor:** MinIO, Inc. & Open Source Community\n> **Fonte:** github.com/minio/minio/tree/master/docs\n\n---\n"
    ]

    for root, dirs, files in sorted(os.walk(MINIO_DOCS_DIR)):
        for f in sorted(files):
            if f.endswith('.md') and not f.startswith('.'):
                fpath = os.path.join(root, f)
                with open(fpath, 'r', encoding='utf-8', errors='ignore') as fp:
                    content = fp.read()
                rel = os.path.relpath(fpath, MINIO_DOCS_DIR)
                title = rel.replace('.md', '').replace('/README', '').replace('/', ' > ').replace('_', ' ').replace('-', ' ').title()
                sections.append({'title': title, 'markdown': content})
                full_md.append(f"# {title}\n\n{content}\n\n---\n")

    out_md = os.path.join(DB_DIR, "minio_object_storage_architecture_guide.md")
    out_pdf = os.path.join(DB_DIR, "minio_object_storage_architecture_guide.pdf")
    ...
    render_book_pdf(
        ...
        "Distributed Architecture, Erasure Coding, Bucket Lifecycle, Site Replication & Multi-Tenancy",
        "MinIO, Inc.",
